...
def createConf(filename, dim):
    ...
    with open(filepath) as csvfile:
        ...
        for row in data.itertuples():
            ...
            if row[dim + 1] == 0: ## 0 is one between eastern or western (we choose an arbitrary one to be protected)
                nonProtected.append(Candidate(qualification, [], id))
            else:
                protected.append(Candidate(qualification, ["western"], id))
            id += 1
